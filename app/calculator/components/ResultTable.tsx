export function ResultTable({ rows, results }: { rows: any[]; results: any[] }) {
  // row_id in results = String(_row_idx) assigned in page.tsx
      ...
      <table className="min-w-full text-sm border-collapse">
        <thead className="sticky top-0 bg-slate-700 text-white">
          <tr>
            ...
            <th className="p-3 border border-slate-600 text-right">Коробок</th>
        ...
        <tbody>
          {rows.map((r, idx) => {
            const res = map.get(String(r._row_idx));
            ...
            return (
              ...
              >
                ...
                <td className={`p-2 border text-right font-semibold ${isError ? "text-gray-400" : "text-green-700"}`}>
                  {res?.total_price != null
                    ? Number(res.total_price).toLocaleString("ru-RU")
                    : "—"}
                </td>
